...
func NewSOCKS5ServerWithResolver(config interfaces.ServerConfig, resolver resolver.NameResolver) *SOCKS5Server {
	...
	if err != nil {
		// 如果创建失败，返回nil
		log.Printf("[SOCKS5-SERVER] Failed to create SOCKS5 server: %v\n", err)
		return nil
	}
	...
	server, err = socks5.New(socks5Config)
	if err != nil {
		log.Printf("[SOCKS5-SERVER] Failed to recreate SOCKS5 server with Dial function: %v\n", err)
		return nil
	}
...
func (s *SOCKS5Server) Listen() error {
	...
	if err != nil {
		log.Printf("[SOCKS5-SERVER] Failed to listen on %s: %v\n", s.config.ListenAddr, err)
		return fmt.Errorf("failed to listen on %s: %w", s.config.ListenAddr, err)
	}

	log.Printf("[SOCKS5-SERVER] Server started successfully, listening on %s\n", s.config.ListenAddr)
	log.Printf("[SOCKS5-SERVER] Authentication enabled: %t (%d users configured)\n",
		len(s.authUsers) > 0, len(s.authUsers))
	log.Printf("[SOCKS5-SERVER] Upstream selector enabled: %t\n", s.config.EnableUpstream)
...
func (s *SOCKS5Server) acceptConnections() {
	...
	for {
		select {
		case <-s.shutdown:
			log.Printf("[SOCKS5-SERVER] Shutdown signal received, stopping connection acceptance. Total connections handled: %d\n", connectionCount)
			return
		default:
			...
			if err != nil {
				...
				case <-s.shutdown:
					return
				default:
					log.Printf("[SOCKS5-SERVER] Accept error: %v\n", err)
					continue
				}
			}
			...
			log.Printf("[SOCKS5-SERVER] New connection accepted from %s (total: %d)\n", clientAddr, connectionCount)
...
func (s *SOCKS5Server) HandleConnection(conn net.Conn) error {
	defer func() {
		conn.Close()
		clientAddr := conn.RemoteAddr().String()
		log.Printf("[SOCKS5-CONN] Connection closed for client %s\n", clientAddr)
	}()

	clientAddr := conn.RemoteAddr().String()
	startTime := time.Now()
	log.Printf("[SOCKS5-CONN] New connection from %s at %s\n", clientAddr, startTime.Format("2006-01-02 15:04:05"))

	// 设置超时
	if s.config.Timeout > 0 {
		timeoutDuration := s.config.Timeout
		conn.SetDeadline(time.Now().Add(timeoutDuration))
		log.Printf("[SOCKS5-CONN] Connection timeout set to %v for client %s\n", timeoutDuration, clientAddr)
	}

	// 记录认证配置状态
	if len(s.authUsers) > 0 {
		log.Printf("[SOCKS5-CONN] Authentication required for client %s (%d users configured)\n", clientAddr, len(s.authUsers))
	} else {
		log.Printf("[SOCKS5-CONN] No authentication required for client %s\n", clientAddr)
	}
	...
	if err != nil {
		log.Printf("[SOCKS5-CONN] Connection failed for client %s after %v: %v\n", clientAddr, duration, err)
	} else {
		log.Printf("[SOCKS5-CONN] Connection completed successfully for client %s, duration: %v\n", clientAddr, duration)
	}
	return err
}
...
func (s *SOCKS5Server) Authenticate(username, password string) bool {
	if s.authUsers == nil || len(s.authUsers) == 0 {
		log.Printf("[SOCKS5-AUTH] No authentication configured, allowing access for user '%s'\n", username)
		return true // 如果没有配置用户，则允许所有连接
	}

	storedPassword, exists := s.authUsers[username]
	if !exists {
		log.Printf("[SOCKS5-AUTH] Authentication failed: user '%s' not found in configured users\n", username)
		return false
	}

	if storedPassword == password {
		log.Printf("[SOCKS5-AUTH] Authentication successful for user '%s'\n", username)
		return true
	} else {
		log.Printf("[SOCKS5-AUTH] Authentication failed: invalid password for user '%s'\n", username)
		return false
	}
}
...
func (s *SOCKS5Server) SelectUpstreamConnection(targetHost string, targetPort int) (net.Conn, error) {
	...
	if s.selector != nil {
		...
		if err != nil {
			log.Printf("[SOCKS5-UPSTREAM] Upstream selector failed for target %s: %v\n", targetAddr, err)
		} else {
			log.Printf("[SOCKS5-UPSTREAM] Upstream selector succeeded for target %s\n", targetAddr)
		}
		return conn, err
	}
	...
	if s.config.EnableUpstream && len(s.config.UpstreamConfig) > 0 {
		log.Printf("[SOCKS5-UPSTREAM] Using direct upstream config for target %s\n", targetAddr)
	...
	log.Printf("[SOCKS5-UPSTREAM] Using direct connection for target %s (timeout: %v)\n", targetAddr, timeout)
	conn, err := net.DialTimeout("tcp", net.JoinHostPort(targetHost, fmt.Sprint(targetPort)), timeout)
	if err != nil {
		log.Printf("[SOCKS5-UPSTREAM] Direct connection failed for target %s: %v\n", targetAddr, err)
	} else {
		log.Printf("[SOCKS5-UPSTREAM] Direct connection established for target %s\n", targetAddr)
	}
	return conn, err
}
...
func (s *SOCKS5Server) Shutdown() error {
	log.Printf("[SOCKS5-SERVER] Initiating graceful shutdown...\n")
	close(s.shutdown)
	if s.listener != nil {
		log.Printf("[SOCKS5-SERVER] Closing listener...\n")
		s.listener.Close()
	}
	log.Printf("[SOCKS5-SERVER] Waiting for active connections to close...\n")
	s.wg.Wait()
	log.Printf("[SOCKS5-SERVER] Server shutdown completed successfully\n")
	return nil
}

// ReloadConfig 重新加载配置
func (s *SOCKS5Server) ReloadConfig(newConfig interfaces.ServerConfig) error {
	log.Printf("[SOCKS5-SERVER] Reloading configuration...\n")
	...
	if newConfig.EnableUpstream {
		...
		s.selector = nil
		log.Printf("[SOCKS5-SERVER] Upstream selector disabled\n")
	}
	...
	server, err := socks5.New(socks5Config)
	if err != nil {
		log.Printf("[SOCKS5-SERVER] Failed to create new SOCKS5 server: %v\n", err)
		return fmt.Errorf("failed to create new SOCKS5 server: %w", err)
	}

	s.server = server

	log.Printf("[SOCKS5-SERVER] Configuration reloaded successfully\n")
	log.Printf("[SOCKS5-SERVER] Authentication users: %d\n", len(newConfig.AuthUsers))
	log.Printf("[SOCKS5-SERVER] Upstream enabled: %t\n", newConfig.EnableUpstream)
	log.Printf("[SOCKS5-SERVER] Listen address: %s\n", newConfig.ListenAddr)
	log.Printf("[SOCKS5-SERVER] Timeout: %v\n", newConfig.Timeout)
