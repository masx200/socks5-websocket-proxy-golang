...
func TestProxyServer(t *testing.T) {
	...
	testResults = append(testResults, "")
	testResults = append(testResults, "执行命令: `curl -v -I http://www.so.com -x socks5://localhost:1080`")
	testResults = append(testResults, "")

	// 创建curl进程
	curlCmd2 := exec.Command("curl", "-v", "-I", "http://www.so.com", "-x", "socks5://localhost:1080")
	// 创建缓冲区来捕获curl输出
